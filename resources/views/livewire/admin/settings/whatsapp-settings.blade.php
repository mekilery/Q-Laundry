            ...
            <div class="card">
                <div class="card-body p-3">
                    <form class="row g-3 align-items-center" wire:submit.prevent="save">
                        ...
                        <hr>
                        <div class="col-md-4">
                            <label class="form-label">{{ $lang->data['account_sid'] ?? 'Account SID' }}<span
                                    class="text-danger">*</span></label>
                            <input type="text" required autofocus class="form-control" wire:model="accountsid">
                            @error('accountsid')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">{{ $lang->data['auth_token'] ?? 'Auth Token' }}<span
                                    class="text-danger">*</span></label>
                            <input type="text" required autofocus class="form-control" wire:model="auth_token">
                            @error('auth_token')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                                ...
                                <div class="row">
                                    <template x-for="(replace,index) in replacers">
                                        <div class="col-12 col-lg-6 col-md-12 mt-2">
                                            <button class="btn-sm btn btn-secondary w-100 h-100 " x-text="replace"
                                                type="button" @click="addText(index,1)"></button>
                                        </div>
                                    </template>
                                </div>
                            </div>
                        ...
                        <div class="row mt-2">
                            <div class="col-12 col-md-12 col-lg-8 col-sm-12">
                                ...
                                <textarea name="text" class="form-control" id="" cols="30" x-ref="status" rows="12"
                                    x-model="myStatusChange"> </textarea>
                            </div>
                            <div class="mt-4 col-12 col-lg-4 col-md-12">
                                <div class="row">
                                    <template x-for="(replace,index) in replacers">
                                        <div class="col-12 col-lg-6 col-md-12 mt-2">
                                            <button class="btn-sm btn btn-secondary w-100 h-100 " x-text="replace"
                                                type="button" @click="addText(index,2)"></button>
                                        </div>
                                    </template>
                                </div>
                            </div>
