                            ...
                            <div class="row align-items-center px-2">
                                <div class="col-8">
                                    <div class="numbers py-2">
                                        <p class="text-sm mb-3 text-uppercase">
                                ...
                                <div class="{{ getOrderStatusWithColor($item->status) }}">
                                    <div class="d-flex justify-content-between mb-2">
                                        <a href="{{ route('admin.view_single_order', $item->id) }}" type="button">
                                            <span
                                                class="fw-600 ms-2 text-dark text-xs mb-0">{{ $item->customer_name ?? ($lang->data['walk_in_customer'] ?? 'Walk In Customer') }}</span>
                                        </a>
                                        <a href="{{ route('admin.view_single_order', $item->id) }}" type="button">
                                            <span
                                                class="fw-600 text-dark text-xs me-2">{{ $item->order_number }}</span>
                                        </a>
                                    </div>
                                    @php
                    ...
                    <div class="row">
                        ...
                        <div class="col-12">
                            <div class="d-flex justify-content-between">
                                <span class="badge badge-md badge-dot ms-3 text-start">
                                    <i class="bg-danger" style="color:  #f5365c;"></i>
                                    <span class="text-dark text-xs">{{ $lang->data['pending'] ?? 'Pending' }}</span>
                                </span>
                                <span class="badge badge-md badge-dot me-3 text-start">
                                    <i class="bg-warning"></i>
                                    <span
                                        class="text-dark text-xs">{{ $lang->data['processing'] ?? 'Processing' }}</span>
                                </span>
                                <span class="badge badge-md badge-dot ms-3 text-start">
                                    <i class="bg-success"></i>
                                    <span
                                        class="text-dark text-xs">{{ $lang->data['ready_to_deliver'] ?? 'Ready To Deliver' }}</span>
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            new Chart(ctx3, {
                ...
                data: {
                    datasets: [{
                        ...
                        pointRadius: 2,
                        borderWidth: 2,
                        backgroundColor: ['#f5365c', '#faae42', '#2dce89'],
                        data: JSON.parse(chartdata),
                        fill: false
                    }],
                    labels: ['Pending', 'Processing', 'Ready to Deliver'],
                },
                options: {
                    responsive: true,
