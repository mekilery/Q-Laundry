                    ...
                    <div class="card-body p-0">
                                ...
                                <tbody>
                                    @foreach ($orders as $row)
                                        <tr>
                                            ...
                                            <td>
                                                <p class="text-xs px-3 font-weight-bold mb-0">
                                                    {{ getCurrency() }}{{ number_format($row->sub_total, 3) }}</p>
                                            </td>
                                            <td>
                                                <p class="text-xs px-3 font-weight-bold mb-0">
                                                    {{ getCurrency() }}{{ number_format($row->addon_total, 3) }}</p>
                                            </td>
                                            <td>
                                                <p class="text-xs px-3 font-weight-bold mb-0">
                                                    {{ getCurrency() }}{{ number_format($row->discount, 3) }}</p>
                                            </td>
                                            <td>
                                                <p class="text-xs px-3 font-weight-bold mb-0">
                                                    {{ getCurrency() }}{{ number_format($row->tax_amount, 3) }}</p>
                                            </td>
                                            <td>
                                                <p class="text-xs px-3 font-weight-bold mb-0">
                                                    {{ getCurrency() }}{{ number_format($row->total, 3) }}</p>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="row align-items-center px-4 mb-3 pt-5">
                            ...
                            <div class="col">
                                ...
                                <span
                                    class="text-sm text-dark ms-2 fw-600 mb-0">{{ getCurrency() }}{{ number_format($orders->sum('total'), 3) }}</span>
                            </div>
                            <div class="col">
                                <span class="text-sm mb-0 fw-500">{{$lang->data['total_tax_amount'] ?? 'Total Tax Amount'}}:</span>
                                <span
                                    class="text-sm text-dark ms-2 fw-600 mb-0">{{ getCurrency() }}{{ number_format($orders->sum('tax_amount'), 3) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
